...
class EnsembleModel:
    ...
    def train(self, X: np.ndarray, y: np.ndarray, 
        ...
        n_available = len(unique_subjects)
        
        # === FIX: 被験者数に応じて検証被験者数を動的に調整 ===
        if n_available < 1:
            raise ValueError(
                f"No subjects available for training after excluding test subject {test_subject}."
            )
        
        # 被験者が1人しかいない場合: サンプルベースで分割
        if n_available == 1:
            if verbose:
                print(f"  [WARNING] Only 1 subject available for training.")
                print(f"  [WARNING] Using sample-based train/val split instead of subject-based.")
            use_sample_split = True
            actual_n_val = 0
        else:
            use_sample_split = False
            # 検証被験者数を調整（最大で利用可能被験者数の半分、最低1人）
            actual_n_val = min(n_val_subjects, max(1, n_available // 2))
            n_train_subjects = n_available - actual_n_val
            
            if verbose and actual_n_val != n_val_subjects:
                print(f"  [Warning] Adjusted n_val_subjects: {n_val_subjects} -> {actual_n_val} "
                      f"(available subjects: {n_available})")
            
            if verbose:
                print(f"  Training subjects: {n_train_subjects}, Validation subjects: {actual_n_val}")
        
        np.random.seed(random_seed)
        
        # Get all training data (excluding test subject)
        all_train_mask = subject_ids != test_subject
        X_all = X[all_train_mask]
        y_all = y[all_train_mask]
        subject_ids_train = subject_ids[all_train_mask]
        ...
        for i in range(self.n_members):
            ...
            if use_sample_split:
                ...
                val_subjects = shuffled[:actual_n_val]
                train_subjects = shuffled[actual_n_val:]
                
                # Get train and validation indices
                train_mask = np.isin(subject_ids_train, train_subjects)
                val_mask = np.isin(subject_ids_train, val_subjects)
            ...
            # === FIX: データが空でないことを確認 ===
            if len(X_train) == 0:
                raise ValueError(f"Training data is empty!")
            if len(X_val) == 0:
                raise ValueError(f"Validation data is empty!")
...
def run_leave_one_subject_out_cv(X: np.ndarray, y: np.ndarray, 
    ...
    # === FIX: 被験者数の警告 ===
    if n_subjects < 3:
        print(f"\n[WARNING] Only {n_subjects} subjects available.")
        print(f"  - Leave-one-subject-out CV requires at least 3 subjects")
        print(f"  - With {n_subjects} subjects: test=1, train/val={n_subjects-1}")
        print(f"  - Results may be unreliable due to limited training data.\n")
    
    if n_subjects < 6:
        print(f"[INFO] Ensemble validation splits adjusted for {n_subjects} subjects.")
        print(f"  - Original n_val_subjects=4 may be reduced automatically.\n")
